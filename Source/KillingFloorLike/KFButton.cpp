...
UKFButton::UKFButton(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	if (HoverSoundAsset.Succeeded())
	{
		...
		ButtonStyle.SetHoveredSound(SlateButtonPressSound);
	}
	if (ClickSoundAsset.Succeeded())
	{
		FSlateSound SlateButtonPressSound = FSlateSound();
		SlateButtonPressSound.SetResourceObject(ClickSoundAsset.Object);
		ButtonStyle.SetPressedSound(SlateButtonPressSound);
	}
